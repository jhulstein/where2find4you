...
test("combined score keeps text relevance ahead of distance and sponsorship", () => {
  assert.match(sql, /normalized_name = q then 70/i);
  assert.match(sql, /normalized_name like q \|\| '%' then 60/i);
  assert.match(sql, /then 50/i);
  assert.match(sql, /then 40/i);
  assert.match(sql, /then 30/i);
  assert.match(sql, /distance_score \+/i);
  assert.match(sql, /least\(popularity_score, 100\)/i);
  assert.match(sql, /least\(coalesce\(sponsored_priority, 0\), 3\)/i);
