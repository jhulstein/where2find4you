...
const categoryAliases = {
  ...
  burger: ["restaurant", "food", "lunch", "dinner"],
...
const termExpansions = {
  ...
  eatery: ["restaurant", "dining", "food"],
  free: ["complimentary", "included", "public", "no cost"],
  dinner: ["restaurant", "food"],
  food: ["restaurant", "market"],
  harbor: ["harbour", "waterfront", "marina", "port", "dock", "view"],
  harbour: ["harbor", "waterfront", "marina", "port", "dock", "view"],
  ...
  kafe: ["cafe", "coffee"],
  kafé: ["cafe", "coffee"],
  lunch: ["restaurant", "food", "meal"],
  marina: ["boats", "dock", "harbor", "harbour", "waterfront"],
  marinas: ["marina", "boats", "dock", "harbor", "harbour", "waterfront"],
  middag: ["dinner"],
...
function textMatchesAnyIntentTerm(text, terms) {
  return terms
    .map(normalizeQuery)
    .filter(Boolean)
    .some((term) =>
      term.includes(" ")
        ? text.includes(term)
        : termMatchesText(text, singularize(term)),
    );
}

function broadIntentProfile(query) {
  const normalized = normalizeQuery(query);

  return {
    hasFoodIntent: textMatchesAnyIntentTerm(normalized, foodIntentTerms),
    hasRomanticIntent: textMatchesAnyIntentTerm(normalized, romanticIntentTerms),
    hasWaterfrontIntent: textMatchesAnyIntentTerm(normalized, waterfrontIntentTerms),
    normalized,
  };
}

export function isBroadIntentSearch(query) {
  const profile = broadIntentProfile(query);

  return (
    profile.hasFoodIntent &&
    (profile.hasRomanticIntent || profile.hasWaterfrontIntent)
  );
}

function placeMatchesBroadIntentQuery(place, query) {
  const profile = broadIntentProfile(query);
  ...
  const text = textForStrictKeywordMatch(place);
  const foodMatch =
    placeMatchesFilterId(place, "restaurants") ||
    textMatchesAnyIntentTerm(text, foodIntentTerms);
  const romanticMatch =
    profile.hasRomanticIntent && textMatchesAnyIntentTerm(text, romanticIntentTerms);
  const waterfrontMatch =
    profile.hasWaterfrontIntent && textMatchesAnyIntentTerm(text, waterfrontIntentTerms);

  return foodMatch && (romanticMatch || waterfrontMatch);
}
...
export function rankPlaces(places, options = {}) {
  ...
  const exploratoryIntent = isBroadIntentSearch(normalizedQuery);
  ...
  return places
    .map((place, index) => {
      ...
      const exploratoryIntentScore =
        exploratoryIntent && placeMatchesBroadIntentQuery(place, normalizedQuery) ? 520 : 0;
...
export function searchPlaceRecords(places, options = {}) {
  ...
  const maxRadiusKm = Number.isFinite(options.maxRadiusKm)
    ? Math.max(0.1, Math.min(Number(options.maxRadiusKm), 100))
    : DEFAULT_SEARCH_RADIUS_KM;
  const matchesBaseFilters = (place) =>
  ...
  const strictFiltered = places.filter(
    (place) =>
      matchesBaseFilters(place) &&
      placeMatchesKeywordQuery(place, normalizedQuery, {
        location: options.location ?? null,
      }),
  );
  const shouldBroadenIntent =
    isBroadIntentSearch(normalizedQuery) && strictFiltered.length < Math.min(4, limit);
  const filtered = shouldBroadenIntent
    ? places.filter(
        (place) =>
          matchesBaseFilters(place) &&
          (placeMatchesKeywordQuery(place, normalizedQuery, {
            ...
            placeMatchesBroadIntentQuery(place, normalizedQuery)),
      )
    : strictFiltered;
